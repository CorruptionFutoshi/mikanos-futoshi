...
struct MemoryMap {
	...
	UINTN buffer_size;
	// VOID means any type like var of Java. represent first address of this struct memorymap
	VOID* buffer;
...
EFI_STATUS GetMemoryMap(struct MemoryMap* map){
	if (map->buffer == NULL){
		// i dont know why if map->buffer equals NULL, derive that buffer is too small. 
...
EFI_STATUS SaveMemoryMap(struct MemoryMap* map, EFI_FILE_PROTOCOL* file){
	...
	Print(L"map->buffer = %08lx, map->map_size = %08lx|n", map->buffer, map->map_size);

	EFI_PHYSICAL_ADDRESS iter.
	int i;

	for(iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0;
 	    iter < (EFI_PHYSICAL_ADDRESS)map->buffer + map_>map_size;
