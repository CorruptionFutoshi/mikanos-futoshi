...
void CopyUTF8String(char* dst, size_t dst_size, const char* src, size_t src_size, int w, int tab) {
	int x = 0;
...
void DrawLines(const LinesType& lines, int start_line, uint64_t layer_id, int w, int h, int tab) {
	char buf[1024];
...
std::tuple<bool, int> WaitEvent() {
	...
	while (true) {
		...
		if (events[0].type == AppEvent::kQuit) {
			return {true, 0};
		} else if (events[0].type == AppEvent::kKeyPush && events[0].arg.keypush.press) {
...
bool UpdateStartLine(int* start_line, int height, size_t num_lines) {
	while (true) {
		const auto [ quit, keycode] = WaitEvent();
		...
		if ((diff < 0 && *start_line == 0) || (diff > 0 && *start_line == num_lines - height)) {
			continue;
		}
