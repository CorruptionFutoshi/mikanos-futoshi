...
namespace fat {
	...
	uintptr_t GetClusterAddr(unsigned long cluster) {
		// fat_size_32 represent num of block per fat;
		// the reason why (cluster - 2) is that clusternumber begin from 2;
	...
	void ReadName(const DirectoryEntry& entry, char* base, char* ext) {
		...
		// delete space of last of name. (0x20 represent space)
	...
	bool NameIsEqual(const DirectoryEntry& entry, const char* name) {
		...
		for (; name[i] != 0 && i83 < sizeof(name83); ++i, ++i83) {
			...
			// toupper(a) return uppercase of a;
