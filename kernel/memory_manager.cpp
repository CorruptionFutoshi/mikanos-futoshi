...
MemoryStat BitmapMemoryManager::Stat() const {
	...
	for (int i = range_begin_.ID() / kBitsPerMapLine; i < range_end_.ID() / kBitsPerMapLine; ++i) {
...
bool BitmapMemoryManager::GetBit(FrameID frame) const {
	...
	// alloc_map_ is not two dimensional array. just array of unsigned long(64bit).
...
// to use global variable written in C in other file, we use extern "C".
...
namespace {
	...
	Error InitializeHeap(BitmapMemoryManager& memory_manager) {
		// 64 * 512 = 128 * 256. one Frame is 4KiB. so it's 128 Mib. 
