...
Task& Task::InitContext(TaskFunc* f, int64_t data) {
	...
	// i don't know why not use ~0x7lu.
...
Error TaskManager::SendMessage(uint64_t id, const Message& msg) {
	...
	// (*it) represent pointer of object that specified be iterator. if there is no (), it return pointer of it->SendMessge().
	(*it)->SendMessage(msg);
	return MAKE_ERROR(Error::kSuccess);
}
...
void InitializeTask() {
	...
	__asm__("cli");
	timer_manager->AddTimer(Timer{timer_manager->CurrentTick() + kTaskTimerPeriod, kTaskTimerValue, 1});
	__asm__("sti");
}
