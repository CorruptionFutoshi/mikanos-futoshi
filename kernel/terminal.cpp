...
Rectangle<int> Terminal::InputKey(uint8_t modifier, uint8_t keycode, char ascii) {
	...
	if (ascii == '\n') {
		...
		cursor_.x = 0;
		// linebuf_[linebuf_index] = 0 is inserting null character to string. so, &linebuf_[0] is string before put enter.
		// Log(kWarn, "line: %s\n", &linebuf_[0]);
...
void Terminal::Scroll1() {
	Rectangle<int> move_src{
		ToplevelWindow::kTopLeftMargin + Vector2D<int>{4, 4 + 16}, {8 * kColumns, 16 * (kRows - 1)}
	};
...
void Terminal::ExecuteLine() {
	...
	// strchr return pointer of second parameter from first parameter.
	char* first_arg = strchr(&linebuf_[0], ' ');
	char* redir_char = strchr(&linebuf_[0], '>');
	char* pipe_char = strchr(&linebuf_[0], '|');

	if (first_arg) {
		// set null character to separate command.
		*first_arg = 0;
		++first_arg;
	}
	...
	if (strcmp(command, "echo") == 0) {
		...
		for (int i = 0; i < pci::num_device; ++i) {
			...
			PrintToFD(*files_[1], "%02x:%02x.%d vend=%04x head=%02x class=%02x.%02x.%02x\n",
		...
		task_manager->NewTask().InitContext(TaskTerminal, reinterpret_cast<int64_t>(term_desc)).Wakeup();
...
void Terminal::Print(char32_t c) {
	...
	// cursor_ and Scroll1() is short form of this->cursor_ and this->Scroll1(). so this is needed.
	auto newline = [this]() {
		cursor_.x = 0;
	...
	if (c == U'\n') {
		...
		// if (show_window_) {
			// why use Writer insted of InnerWriter() is that CalcCursorPos() return position based on window topleft.
